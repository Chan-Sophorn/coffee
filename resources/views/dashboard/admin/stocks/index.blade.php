...
@section('content')
    <div class="container mt-5">
        <div class="row" style="padding-left: 70px; padding-right: 70px;">
            <h4 class="bg-primary text-white p-2">Stock Coffee</h4>
                    ...
                    @if ($stocks->count() > 0)
                        @foreach ($stocks as $index => $item)
                            <tr>
                                ...
                                <td>{{ "$" . ' ' . $item->total }}</td>
                                <td>
                                    <a href="{{ route('admin.stockCoffee.edit', $item->id) }}"><i
                                    ...
                                    <form id="delete-from-{{ $item->id }}"
                                        action="{{ route('admin.stockCoffee.destroy', $item->id) }}" method="POST"
                                        style="display: none;">
                                        @csrf
                                        @method('DELETE')
                                    </form>
